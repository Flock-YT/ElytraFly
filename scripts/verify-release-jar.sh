...
if ! printf '%s\n' "$jar_entries" | grep -Eq '^plugin\.yml$'; then
    echo "plugin.yml is missing from $jar_path." >&2
    exit 1
fi

if ! printf '%s\n' "$jar_entries" | grep -Eq '^config\.yml$'; then
    echo "config.yml is missing from $jar_path." >&2
    exit 1
fi
...
if ! grep -Eq "^api-version: ['\"]?1\\.13['\"]?$" "$temporary_directory/plugin.yml"; then
    echo "plugin.yml must declare api-version 1.13 for the supported compatibility range." >&2
    exit 1
fi
...
echo "Verified $jar_path (version $expected_version, Java 8 bytecode, required resources present, no bundled Bukkit API or signatures)."

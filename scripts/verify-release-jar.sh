...
while IFS= read -r -d '' class_file; do
    ...
    if (( major_version > 65 )); then
        relative_class="${class_file#"$temporary_directory"/}"
        echo "Class $relative_class has major version $major_version; maximum allowed is 65 (Java 21)." >&2
        exit 1
    fi
done < <(find "$temporary_directory" -type f -name '*.class' -print0)

echo "Verified $jar_path (version $expected_version, Java 21 bytecode, required resources present, no bundled Bukkit API or signatures)."
